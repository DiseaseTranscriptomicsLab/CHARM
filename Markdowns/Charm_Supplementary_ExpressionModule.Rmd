...
#lets save the genes which we maintain
impgenes <- unique(c(row.names(falternew), unique(Charm.object.K562$Experiment)))
...
saveRDS(Charm.object.K562, "../data/Charm.object_K562.RDS")
```
...
for (rbp in unique(Charm.object.K562$Experiment)) {
  Charm.object.K562$DEGenes[[rbp]] <- plot_rbp_volcano(Charm.object.K562, rbp)$top_table
}
...
for (rbp in unique(Charm.object.K562$Experiment)) {
  Charm.object.K562$GSEA[[rbp]] <- plot_gsea(Charm.object.K562, rbp)$geneset_table
}
...
#lets save the genes which we maintain
impgenes <- unique(c(row.names(falternew), unique(Charm.object.HEPG2$Experiment)))
...
saveRDS(Charm.object.HEPG2, "../data/Charm.object_HEPG2.RDS")
```
...
for (rbp in unique(Charm.object.HEPG2$Experiment)) {
  Charm.object.HEPG2$DEGenes[[rbp]] <- plot_rbp_volcano(Charm.object.HEPG2, rbp)$top_table
}
...
for (rbp in unique(Charm.object.HEPG2$Experiment)) {
  Charm.object.HEPG2$GSEA[[rbp]] <- plot_gsea(Charm.object.HEPG2, rbp)$geneset_table
}
...
get_rbp_tvals <- function(Charmobj, rbp) {
  ...
  rbp_data <- Charmobj$DEGenes[[rbp]]  # extract the data.frame for that RBP
