...
saveRDS(Charm.object.binding.filtered, "../intermediary_files/Charm.binding.filtered.both_new_full.Rds")
```
...
data.table::fwrite(
  oddsrat_df,
  "../intermediary_files/Charm.binding.filtered.both_new_full.csv",
  sep = "\t"
)
...
For computational intensive processes, python was used. A copy of the script is present in the charm folder.

### Wassertein Loading

```{r}
Oddsratinc_was <- fread("../intermediary_files/Wasserstein_oddsratinc.csv")
Oddsratinc_spe <- fread("../intermediary_files/Spearman_oddsratinc.csv")
Oddsratinc_pea <- fread("../intermediary_files/Pearson_oddsratinc.csv")

x <- Oddsratinc_was[[ncol(Oddsratinc_was)]]
y <- Oddsratinc_spe[[ncol(Oddsratinc_spe)]]
z <- Oddsratinc_pea[[ncol(Oddsratinc_pea)]]
...
plot_df$Spearman_01 <- (plot_df$Spearman + 1) / 2
plot_df$Pearson_01 <- (plot_df$Pearson + 1) / 2


plot_df$Metric <- plot_df$Wasserstein/plot_df$Pearson

ggplot(plot_df, aes(x = (Pearson))) +
  geom_density(
    linewidth = 1,
    fill = "darkgreen",
    alpha = 0.4
...
## Convertion into "distance"

```{r}
dt <- copy(Oddsratinc)  # or Oddsratdec

# Create RBP×Target identifiers
dt[, Pair1 := paste(RBP1, Target1, sep = "|")]
dt[, Pair2 := paste(RBP2, Target2, sep = "|")]

# Aggregate over dPSI (keep strongest similarity)
dt_pair <- dt[
  ,
  .(Distance = min(Wasserstein, na.rm = TRUE)),
  by = .(Pair1, Pair2)
]

pairs <- unique(c(dt_pair$Pair1, dt_pair$Pair2))
n <- length(pairs)
pair_index <- setNames(seq_len(n), pairs)

i <- pair_index[dt_pair$Pair1]
j <- pair_index[dt_pair$Pair2]
x <- dt_pair$Distance
...
mds_df <- data.frame(
  MDS1 = mds_coords$points[,1],
  MDS2 = mds_coords$points[,2]
)

sum(mds_coords$eig[1:2]) / sum(abs(mds_coords$eig))

mds_df$RBP <- Oddsratinc$RBP1


rbp_mds <- as.data.table(mds_df)[
  ,
  .(
    MDS1 = mean(MDS1),
    MDS2 = mean(MDS2),
...
```{r fig.height=20, fig.width=20}
rbp_mds <- readRDS("/mnt/scratch/home/alexandre/Projects/CHARM/intermediary_files/MDS_both_oddsratinc.rds")
ggplot(rbp_mds, aes((MDS1), (MDS2))) +
  geom_point(aes(size = n_pairs), alpha = 0.7) +
  geom_text(aes(label = RBP), size = 3, vjust = -1) +
  theme_minimal()
```
...
```{r fig.height=20, fig.width=20}
# Load pairwise distances (already aggregated per Pair1-Pair2)
dt_pair <- fread(
  "/mnt/scratch/home/alexandre/Projects/CHARM/intermediary_files/Wasserstein_oddsratinc.csv"
)

sim_vec <- dt_pair$Wasserstein

sim_vec <- as.data.frame(sim_vec)

ggplot(sim_vec, aes(x=sim_vec)) + geom_density() + theme_bw() + ylab("Events") + xlab("Similarity") +
  theme(axis.line = element_line(colour = "black"),
        panel.grid.minor = element_blank(),
        panel.border = element_blank(),
        panel.background = element_blank()) 
# Create pair identifiers
dt_pair[, Pair1 := paste(RBP1, Target1, sep = "|")]
dt_pair[, Pair2 := paste(RBP2, Target2, sep = "|")]

# Extract RBPs
dt_pair[, RBP_A := RBP1]
dt_pair[, RBP_B := RBP2]

# Convert similarity → distance if needed
max_sim <- max(dt_pair$Wasserstein, na.rm = TRUE)
dt_pair[, Distance := max_sim - Wasserstein]

# Aggregate to RBP × RBP
rbp_dist <- dt_pair[
  ,
...
rbps <- sort(unique(c(rbp_dist$RBP_A, rbp_dist$RBP_B)))
n <- length(rbps)
rbp_index <- setNames(seq_len(n), rbps)

D_rbp <- matrix(NA_real_, n, n, dimnames = list(rbps, rbps))

i <- rbp_index[rbp_dist$RBP_A]
j <- rbp_index[rbp_dist$RBP_B]

D_rbp[cbind(i, j)] <- rbp_dist$Distance
D_rbp[cbind(j, i)] <- rbp_dist$Distance
diag(D_rbp) <- 0

# Missing = maximally distant
max_dist <- max(D_rbp, na.rm = TRUE)
D_rbp[is.na(D_rbp)] <- max_dist

# Convert to dist object
rbp_dist_obj <- as.dist(D_rbp)

# Hierarchical clustering
hc <- hclust(rbp_dist_obj, method = "average")  # average = UPGMA

pheatmap(
  D_rbp,
  clustering_distance_rows = rbp_dist_obj,
  clustering_distance_cols = rbp_dist_obj,
  clustering_method = "ward.D2",
  show_rownames = T,
  show_colnames = T,
  color = colorRampPalette(c("navy", "white", "firebrick"))(100)
...
S_rbp <- max(D_rbp) - D_rbp

pheatmap(
  S_rbp,
  clustering_distance_rows = as.dist(D_rbp),
  clustering_distance_cols = as.dist(D_rbp),
  clustering_method = "ward.D2",
  color = colorRampPalette(c("white", "orange", "red"))(100)
)

D_scaled <- t(scale(t(D_rbp)))
...
```{r fig.height=20, fig.width=20}
rbp_mds <- readRDS("/mnt/scratch/home/alexandre/Projects/CHARM/intermediary_files/MDS_both_oddsratdec.rds")
ggplot(rbp_mds, aes((MDS1), (MDS2))) +
  geom_point(aes(size = n_pairs), alpha = 0.7) +
  geom_text(aes(label = RBP), size = 3, vjust = -1) +
  theme_minimal()
...
# Load pairwise distances (already aggregated per Pair1-Pair2)
dt_pair <- fread(
  "/mnt/scratch/home/alexandre/Projects/CHARM/intermediary_files/Wasserstein_oddsratdec.csv"
)
...
```{r fig.height=20, fig.width=20}
rbp_mds <- readRDS("/mnt/scratch/home/alexandre/Projects/CHARM/intermediary_files/MDS_joint_oddsratinc_dec.rds")
ggplot(rbp_mds, aes((MDS1), (MDS2))) +
  geom_point(aes(size = n_pairs), alpha = 0.7) +
  geom_text(aes(label = RBP), size = 3, vjust = -1) +
  theme_minimal()
...
# H. Yet another method

```{r fig.height=200, fig.width=50}
strong_df <- fread("~/Projects/CHARM/intermediary_files/Charm.binding.filtered.both_new.csv")
strong_df_inc <- subset(strong_df, Direction == "oddsratinc")
strong_df_dec <- subset(strong_df, Direction == "oddsratdec")
...
# create row labels
df <- df %>%
  mutate(RowLabel = paste(RBP, Target, sep = "_"))

# select only pos columns
pos_cols <- grep("^pos_", names(df), value = TRUE)

# matrix for heatmap
mat <- df %>%
  select(all_of(pos_cols)) %>%
  as.matrix()

# assign rownames
rownames(mat) <- df$RowLabel
...
vlines <- c(50,250,450,500,550,750,950)
...
decorate_heatmap_body("value", {
  for (x in vlines) {
    grid.lines(
      x = unit(c(x-0.5, x-0.5)/ncol(mat), "npc"),
      y = unit(c(0,1), "npc"),
...
dt$Pair1 <- paste0(dt$RBP1, "_", dt$Target1)
dt$Pair2 <- paste0(dt$RBP2, "_", dt$Target2)

write_csv(dt, "/mnt/scratch/home/alexandre/Projects/CHARM/intermediary_files/FullSimilarity_oddsratinc.csv")
```
